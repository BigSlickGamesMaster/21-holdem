...
class PlayerListener {
  ...
  async onEvent(oDataa, callback = () => {}) {
    ...
    switch (sEventName) {
      ...
      case 'reqSplit':
        ...
        break;
      case 'reqSideBet':
        this.sideBet(oData, participant, callback);
        break;
      default:
  ...
  async sideBet(oData, participant, callback) {
    try {
      await participant.placeSideBet(oData, callback);
    } catch (error) {
      console.log('Error in PlayerListener sideBet method:', error);
      this.logError(error, callback);
    }
  }
