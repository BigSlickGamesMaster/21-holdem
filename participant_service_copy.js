...
class Service {
  constructor(oParticipantData, oBoard) {
    ...
    this.eSplitPhase = oParticipantData.eSplitPhase ?? 'none'; // 'none' | 'hand1' | 'hand2' | 'done'
    this.bMainHandStand = oParticipantData.bMainHandStand ?? false;
    this.bSplitHandStand = oParticipantData.bSplitHandStand ?? false;
    // nLastBidChips is swapped between main and split hands during the split phase so each
    // hand independently tracks its contribution against the table's nMinBet.
    this.nMainHandLastBidChips = oParticipantData.nMainHandLastBidChips ?? 0;
    this.nSplitHandBet = oParticipantData.nSplitHandBet ?? 0; // initial split fee = hand2's opening contribution
    this.aSideBets = oParticipantData.aSideBets ?? []; // side bets placed for this hand: ['flush', 'straight', 'bj']
