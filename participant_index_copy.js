...
class Participant extends Service {
  async call(oData, callback) {
    try {
      ...
      if (this.eSplitPhase === 'hand1' || this.eSplitPhase === 'hand2') return await this.advanceSplitPhase();
  ...
  async check(oData, callback) {
    try {
      ...
      await this.oBoard.saveLogs([{ sAction: 'check', eLogType: 'game', iUserId: this.iUserId }]);
      if (this.eSplitPhase === 'hand1' || this.eSplitPhase === 'hand2') return await this.advanceSplitPhase();
      await this.passTurn();
      return;
    } catch (error) {
  ...
  async takeTurn() {
    try {
      ...
      // During split phase the player still needs to act on each sub-hand even if they stood on the previous one.
      if (this.isDoubleDownLock && this.eSplitPhase !== 'hand1' && this.eSplitPhase !== 'hand2') {
  ...
  async split(oData, callback) {
    try {
      ...
      this.bHasSplit = true;
      this.eSplitPhase = 'hand1'; // player now acts on each sub-hand before passing turn
      // Record the split fee as hand2's opening contribution so takeTurn() correctly
      // computes toCallAmount for each sub-hand independently.
      this.nSplitHandBet = nSplitAmount;
      ...
      return await this.takeTurn(); // player acts on hand1, then hand2 via advanceSplitPhase()
    } catch (error) {
  ...
  async advanceSplitPhase() {
    if (this.eSplitPhase === 'hand1') {
      // Stash main hand's betting contributions; hand2 starts fresh from only the split fee.
      // This ensures takeTurn() computes the correct toCallAmount for hand2 independently.
      this.nMainHandLastBidChips = this.nLastBidChips;
      this.nLastBidChips = this.nSplitHandBet; // split fee is hand2's opening contribution

      // Capture whether main hand was stood before resetting for hand2
      this.bMainHandStand = this.isDoubleDownLock;
      // Clear the stand lock so hand2 turn runs normally in takeTurn()
      this.isDoubleDownLock = false;
      // Restore full action set so takeTurn() can compute the correct actions for hand2
      this.aUserAction = ['c', 'r', 'f', 's', 'd'];
      this.eSplitPhase = 'hand2';
      await this.oBoard.update({ aParticipant: [this.toJSON()] });
      return await this.takeTurn();
    } else {
      // hand2 done — persist whether the split hand was stood
      this.bSplitHandStand = this.isDoubleDownLock;
      // Restore nLastBidChips to main hand's value for the passTurn() round-settled check
      this.nLastBidChips = this.nMainHandLastBidChips;
      // Restore isDoubleDownLock to reflect main-hand stand state for community card logic
      this.isDoubleDownLock = this.bMainHandStand;
      this.eSplitPhase = 'done';
      await this.oBoard.update({ aParticipant: [this.toJSON()] });
      return await this.passTurn();
  // -------------------------------------------------------
    }
  ...
  // Side Bets  (delta-based: nDelta +1 adds a chip, -1 removes)
  // -------------------------------------------------------
  async placeSideBet({ sSideBet, nDelta }, callback) {
    const validBets = ['flush', 'straight', 'bj'];
    if (!validBets.includes(sSideBet)) return callback({ error: 'Invalid side bet type' });

    const MAX_CHIPS = 5;
    const nBetAmount = Number(this.oBoard.nMinBet) || 0;
    const delta = Number(nDelta) >= 0 ? 1 : -1;

    // Count how many of this bet type the player already has
    const nCurrentCount = this.aSideBets.filter(b => b === sSideBet).length;

    if (delta > 0) {
      // Adding a chip
      if (nCurrentCount >= MAX_CHIPS) return callback({ error: 'Maximum chips reached for this bet' });
      if (this.nChips < nBetAmount) return callback({ error: 'Not enough chips' });
      await this.updateUser({ $inc: { nChips: -nBetAmount, nTotalBetAmount: nBetAmount } });
      this.nChips -= nBetAmount;
      this.aSideBets.push(sSideBet);
    } else {
      // Removing a chip
      if (nCurrentCount <= 0) return callback({ error: 'No bet to remove' });
      await this.updateUser({ $inc: { nChips: nBetAmount, nTotalBetAmount: -nBetAmount } });
      this.nChips += nBetAmount;
      const removeIdx = this.aSideBets.lastIndexOf(sSideBet);
      this.aSideBets.splice(removeIdx, 1);
    }

    const nChipCount = this.aSideBets.filter(b => b === sSideBet).length;
    await this.oBoard.update({ aParticipant: [this.toJSON()] });
    await this.emit('resSideBet', { sSideBet, nChipCount, nChips: this.nChips, nBetAmount });
    return callback({});
  }

  // Evaluate side bets — each chip in aSideBets is evaluated & paid separately.
  evaluateSideBets(aCommunityCards) {
    const allCards = [...(this.aCardHand || []), ...(aCommunityCards || [])];
    const PAYOUTS = { flush: 5, straight: 10, bj: 2 };
    const nBetAmount = Number(this.oBoard.nMinBet) || 0;

    // Pre-compute win conditions once per bet type (not per chip)
    const winCache = {};

    const checkFlush = () => allCards.length >= 2 && allCards.every(c => c.eSuit === allCards[0].eSuit);
    const checkStraight = () => {
      const vals = allCards.map(c => { const v = Number(c.nValue); return v === 1 ? 11 : v; }).sort((a, b) => a - b);
      return vals.length >= 2 && vals.every((v, i) => i === 0 || v === vals[i - 1] + 1);
    };
    const checkBJ = () => {
      const pvt = this.aCardHand?.[0];
      const com = aCommunityCards?.[0];
      if (!pvt || !com) return false;
      const vPvt = Number(pvt.nValue) === 1 ? 11 : Number(pvt.nValue);
      const vCom = Number(com.nValue) === 1 ? 11 : Number(com.nValue);
      return vPvt + vCom === 21;
    };

    const getWin = (key) => {
      if (winCache[key] === undefined) {
        if (key === 'flush')    winCache[key] = checkFlush();
        if (key === 'straight') winCache[key] = checkStraight();
        if (key === 'bj')       winCache[key] = checkBJ();
      }
      return winCache[key] ?? false;
    };

    // Aggregate: one result entry per unique bet type, payout = chipCount * multiplier * nBetAmount
    const uniqueBets = [...new Set(this.aSideBets)];
    const results = uniqueBets.map(sSideBet => {
      const nChipCount = this.aSideBets.filter(b => b === sSideBet).length;
      const bWon = getWin(sSideBet);
      const nPayout = bWon ? nBetAmount * (PAYOUTS[sSideBet] || 0) * nChipCount : 0;
      return { sSideBet, bWon, nPayout, nChipCount };
    });
